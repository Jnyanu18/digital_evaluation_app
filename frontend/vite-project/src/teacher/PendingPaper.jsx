...
import { FiArrowLeft, FiClock, FiFileText, FiUser, FiFolder } from "react-icons/fi";
...
const PendingPaper = () => {
  ...
  const fetchPendingPapers = async () => {
    try {
      ...
      addToast({ title: "Error", description: "Failed to load pending papers.", status: "error" });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <AnimatedPage className="container mx-auto px-4 pt-32 max-w-7xl">
        <div className="flex items-center gap-4 mb-8 pb-6 border-b border-[--color-border-default]">
          ...
          <div>
            <h1 className="text-3xl font-heading font-bold mb-1 flex items-center gap-3">
              <FiClock className="text-[--color-accent-orange]" />
              {selectedSection ? `Section ${selectedSection}` : "Pending Papers"}
            </h1>
            <p className="text-[--color-text-secondary]">
              {selectedSection ? "Pending evaluations for this section" : "Select a section to view pending evaluations"}
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="flex justify-center p-12">
            <div className="w-8 h-8 border-4 border-[--color-accent-orange]/20 border-t-[--color-accent-orange] rounded-full animate-spin"></div>
          </div>
        ) : !selectedSection ? (
          ...
          Object.keys(groupedPapers).length === 0 ? (
            <div className="text-center py-20 border border-[--color-border-default] rounded-[--radius-large] bg-[--color-bg-card]">
              <FiClock className="w-12 h-12 text-[--color-text-muted] mx-auto mb-4" />
              <h3 className="text-xl font-medium text-[--color-text-primary] mb-2">No Pending Papers</h3>
              <p className="text-[--color-text-secondary]">You have caught up with all evaluations!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {Object.keys(groupedPapers).map((section) => (
                <GlassCard
                  key={section}
                  className="cursor-pointer group hover:border-[--color-accent-orange]/50 transition-all duration-300 hover:-translate-y-1"
                  onClick={() => handleSectionClick(section)}
                >
                  <div className="flex items-start gap-4 mb-4">
                    <div className="p-3 rounded-lg bg-[--color-accent-orange]/10 text-[--color-accent-orange] group-hover:scale-110 transition-transform">
                      <FiFolder className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="text-xl font-heading font-semibold text-[--color-text-primary] group-hover:text-[--color-accent-orange] transition-colors">
                        Section {section}
                      </h3>
                    </div>
                  </div>
                  <div className="flex items-center justify-between text-sm mt-4 pt-4 border-t border-[--color-border-default]">
                    ...
                    <span className="font-bold text-[--color-accent-orange] bg-[--color-accent-orange]/10 px-2.5 py-1 rounded-md border border-[--color-accent-orange]/20">
          ...
          /* View: List of Pending Papers in Selected Section */
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {groupedPapers[selectedSection].map((paper) => (
              <GlassCard key={paper._id} className="relative group flex flex-col h-full border-[--color-accent-orange]/20">
                <div className="mb-5 pr-4 flex items-start justify-between">
                   <div className="flex items-center gap-2">
                      <div className="p-2 rounded bg-[--color-accent-orange]/10 text-[--color-accent-orange]">
                ...
                <div className="space-y-3 mb-6 flex-1 text-sm bg-[--color-bg-elevated] p-3 rounded-lg border border-[--color-border-default]/50">
                  ...
                  <div className="flex justify-between items-center py-1">
                    <span className="text-[--color-text-muted] flex items-center gap-2">
                       <FiClock className="w-4 h-4" /> Status
                    </span>
                    <span className="font-medium px-2 py-0.5 rounded text-xs uppercase tracking-wider border text-[--color-accent-orange] bg-[--color-accent-orange]/10 border-[--color-accent-orange]/20">
